import * as Stories from './Headline.stories.svelte';
...
The Headline component comes in three sizes: `s`, `m` and `l`. Use the default `m` size in most
cases, `s` for card headers, and `l` for page titles. The largest size is set in the display
typeface.

The legacy `one`, `two`, `three` and `four` sizes are deprecated. They still work and map onto the
new scale — `one` to `l`, `two` to `m`, and both `three` and `four` to `s`.
...
<Canvas of={Stories.SectionNoHeadline} />
...
<Canvas of={Stories.HeadlineNoSection} />
