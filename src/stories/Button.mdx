...
Pass an `href` to render the button as a link, or use the `as` prop to render it as any other element.
Use a link whenever the action takes the user somewhere else, so that it can be opened in a new tab
and shows up in the browser's history.

<Canvas of={Stories.Links} />

### Stretched

Stretch the button across the full width of its parent, for example at the bottom of a form on
mobile.

<Canvas of={Stories.Stretched} />
...
Icon buttons are used when the action is clear and doesn't need a label. They are generally used for actions that are already
clear by the icon itself, such as "delete", "edit", "add", "close", "search", etc.

In order to keep accessibility, use leadingIcon snippet/slot and hideLabel property.
Always add a label, even if it's hidden, to provide context for screen readers.

- **Do** use icons with a size that goes along with the button size.
