import * as Stories from './Body.stories.svelte';
...
The Body component comes in three sizes: `s`, `m` and `l`. Use the default `m` size in most cases.
The legacy `one` and `two` sizes are deprecated, and map onto `m` and `s` respectively.
...
that is semantically correct; it defaults to the semibold weight.

<Canvas of={Stories.Weights} />

### Decorations

Use the `strikethrough` decoration for text that is no longer relevant, such as a previous price.
Underline is reserved for hyperlinks. Use the `as` prop to render the text as the `del` element where
...
The `variant` prop is deprecated. Use `color` instead of the `alert`, `confirm` and `subtle`
variants, `weight` instead of the `highlight` variant, and custom CSS instead of the `quote` variant.

<Canvas of={Stories.Variants} />

### Semantic usage

The Body component accepts semantic children that will be rendered correctly and used with other args.
