...
import "./chunk-AE6N56OT.js";
import "./chunk-6QZZJTN4.js";
import "./chunk-NEVMD4ST.js";
import "./chunk-NGQPZQN3.js";
...
function create_if_block(ctx) {
  ...
  const block = {
    ...
    p: function update(ctx2, dirty) {
      ...
      if (switch_value !== (switch_value = /*decorator*/
      ctx2[0].Component)) {
        if (switch_instance) {
          group_outros();
          const old_component = switch_instance;
...
// node_modules/@storybook/svelte/dist/chunk-L4OAT24I.mjs
function unWrap(obj) {
...
function decorateStory(storyFn, decorators) {
  return decorators.reduce((decorated, decorator) => (context) => {
    let story, decoratedStory = decorator((update) => (story = decorated({ ...context, ...sanitizeStoryContextUpdate(update) }), story), context);
...
function renderToCanvas({ storyFn, kind, name, showMain, showError, storyContext, forceRemount }, canvasElement) {
  let existingComponent = componentsByDomElement.get(canvasElement);
  if (forceRemount && teardown(canvasElement), !existingComponent || forceRemount) {
...
var { configure: coreConfigure, clientApi, forceReRender } = start(renderToCanvas, { decorateStory, render });
var { raw } = clientApi;
var FRAMEWORK = "svelte";
var storiesOf = (kind, m) => clientApi.storiesOf(kind, m).addParameters({ framework: FRAMEWORK });
var configure = (loadable, m) => coreConfigure(FRAMEWORK, loadable, m);
var _a;
